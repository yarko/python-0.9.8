...
class NullFormatter(BaseFormatter):
	...
	def init(self):
		d = NullMeasurer().init()
		b = NullBackEnd().init()
		return BaseFormatter.init(self, d, b)


# Formatting class to write directly to a file
class WritingFormatter(BaseFormatter):
	#
	def init(self, fp, width):
		dm = NullMeasurer().init()
		dw = FileWriter().init(fp)
		b = WritingBackEnd().init(dw, width)
		self = BaseFormatter.init(self, dm, b)
		self.blanklines = 1
		return self
	#
...
class StdwinBackEnd(SavingBackEnd):
	...
	def init(self, window, drawnow):
		...
		return SavingBackEnd.init(self)
	#
	def finish(self):
		self.d.close()
		self.d = None
