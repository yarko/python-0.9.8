...
import sys
sys.path.insert(0, '/ufs/guido/src/www')
import os
import string
import marshal
import getopt
...
class WWWWindow:
	...
	def save(self):
		...
		try:
			...
			return
		self.window.setwincursor(WAITCURSOR)
		fmtr = fmt.WritingFormatter().init(f, 72)
		parser = htmllib.FormattingParser().init(fmtr, \
	...
	def setrawdata(self, addr, data):
		...
		b = fmt.StdwinBackEnd().init(self.window, 0)
		b.d.setfont(htmllib.StdwinStylesheet.stdfontset[0])
		##b = fmt.StdwinBackEnd().init(self.window, 1) # Mac version
		f = fmt.BaseFormatter().init(b.d, b)
		p = htmllib.AnchoringParser().init(f, htmllib.StdwinStylesheet)
