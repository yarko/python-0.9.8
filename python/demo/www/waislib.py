...
class Question:
	...
	def init(self):
		self.seed_words = ''
		self.sources = []
		self.relevant_documents = waisqp.List().init()
		self.result_documents = waisqp.List().init()
	...
	#
	def do_query(self):
		qrec = waisqp.Record().init('question')
		qrec['version'] = '2'
		qrec['seed-words'] = '"' + self.seed_words + '"'
		qrec['relevant-documents'] = self.relevant_documents
		slist = waisqp.List().init()
		for source in self.sources:
			s = waisqp.Record().init('source-id')
			s['filename'] = '"' + source + '"'
			slist.append(s)
		qrec['sources'] = slist
		file = self.scratch
