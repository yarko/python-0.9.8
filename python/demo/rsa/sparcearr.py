...
class SparceArr():
	...
	def init(self, keys, items):
		#print 'SparceArr.init' + `self, keys, items`
		for i in range(len(keys)):
			if keys[i] in keys[:i]:
				raise ValueError, 'sparcearr: double keys'
		return self._init(keys, items)
	...
	def __add__(self, other):
		...
		return SparceArr()._init(self._keys + other._keys, \
			  self._items + other._items)

		
def sparcearr(*arg):
	if len(arg) == 0:
		return SparceArr().init([], [])
	if len(arg) == 1 and type(arg[0]) == type([]):
		return SparceArr().init(range(len(arg[0])), arg[0][:])
	if len(arg) == 2 \
		  and type(arg[0]) == type([]) \
		  and type(arg[1]) == type([]) \
		  and len(arg[0]) == len(arg[1]):
		return SparceArr().init(arg[0][:], arg[1][:])
	if len(arg) > 0 and type(arg[0]) == type(()) and len(arg[0]) == 2:
		...
		for i in arg:
			...
			return SparceArr().init(keys, items)
