...
def factorlist(*pairs):
	return apply(FactorList().init, pairs)
			

class Fnum:

	def init(self, n, status, factorfreebound):
		self.n = n
...
def fnum(n, *rest):
	if len(rest) == 2:
		return Fnum().init(n, rest[0], rest[1])
	if len(rest) == 1:
		return Fnum().init(n, rest[0], 2)
	elif len(rest) != 0:
		raise TypeError, 'fnum(.): too many arguments (>3)'
	return Fnum().init(n, UNKNOWN, 2) 
