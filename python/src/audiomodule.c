...
/* For SGI Personal IRIS 4D/25 under IRIX 3.3; <sys/audio.h> mentions "IP6" */
...
init()
{
	...
	if ((audio_fd = asa_init()) >= 0) {
		/* Make sure subprocesses don't see this file descriptor */
		(void) fcntl(audio_fd, F_SETFD, FD_CLOEXEC);
		return 1;
	}
	...
	object *args;
{
	int n, n2;
	char *buf;
	if (!getargs(args, "s#", &buf, &n))
		return NULL;
	if (!init())
		return NULL;
	errno = 0;
	n2 = write(audio_fd, buf, n);
	if (intrcheck()) {
		err_set(KeyboardInterrupt);
		return NULL;
	}
	...
	object *args;
{
	...
	int i, n;
	int c;
	if (!getargs(args, "(s#ii)", &s, &n, &f1, &f2))
		return NULL;
	v = newsizedstringobject((char *)NULL, n);
	if (v == NULL)
		return NULL;
	...
	object *args;
{
	...
	if (!getargs(args, "(s#s#)", &sa, &na, &sb, &nb))
		return NULL;
	n = (na > nb) ? na : nb;
	v = newsizedstringobject((char *)NULL, n);
	if (v == NULL)
	...
	object *args;
{
	char *buf;
	int size;
 	if (!getargs(args, "s#", &buf, &size))
		return NULL;
	if (stdaudio_buffer != NULL) {
		err_setstr(RuntimeError, "audio.start_playing: device rbusy");
		return NULL;
	}
	asa_start_write(buf, size);
	INCREF(args);
	stdaudio_buffer = args;
	INCREF(None);
	return None;
}
