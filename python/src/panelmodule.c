...
/* Panel module.
   Interface to the NASA Ames "panel library" for the SGI Graphics
   Library by David Tristram.  It has been developed for version 9.7,
   but I believe it works with 9.6 as well if you take a few entries
   out of the 'initializerlist' array below.
   ...
   Compilation note: this module won't compile with gcc since there is
   a definition of calloc() in panel.h which conflicts with <stdlib.h>.
   You can either fix panel.h or use plain cc to compile this one file.
*/
			...
			case T_STRING:
				if (v == None)
					*(char**)addr = NULL;
					...
					/* XXX Can you spot the memory leak? */
					INCREF(v);
					*(char**)addr = getstringvalue(v);
				}
				break;
			case T_ACTUATOR:
				if (v == None)
	...
	object *args;
{
	...
	if (!getstrarg(args, &str))
		return NULL;
	width = pnl_strwidth(self->ob_panel, str);
	...
	object *v;
{
	...
	if (v == NULL) {
		err_setstr(TypeError, "can't delete panel attributes");
		return -1;
	}
	...
	object *v;
{
	...
	if (v == NULL) {
		err_setstr(TypeError, "can't delete actuator attributes");
		return -1;
	}
