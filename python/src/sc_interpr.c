...
/* Forward */
static int xPop PROTO((TscOperand n, struct sc_ProcessBlock *sc_pb));
...
int datasize;
	...
	if (string == NULL)
		return -1;
	sc_pb->data = (unsigned char *) getstringvalue(string);
	if (sc_pb->data == NULL)
		return -1;
...
#ifdef STACK_TRACE
	printf("pushed :");
	if (printobject(element, stdout, 0) != 0)
		err_clear();
	printf("\n");
#endif
	...
	struct sc_ProcessBlock *sc_pb;
{
	...
	printf("\n");
#endif
	ret = trans(&sc_pb->hdr, (bufptr)sc_pb->buffer, (bufsize) sc_pb->bp, 
		&sc_pb->hdr, (bufptr)sc_pb->buffer, (bufsize)sc_pb->maxbufsize);
#ifdef SCDEBUG
...
#ifdef STACK_TRACE
		printf("popped :");
		if (printobject(sc_pb->stack[sc_pb->sp], stdout, 0) != 0)
			err_clear();
		printf("\n");
#endif
	...
	struct sc_ProcessBlock *sc_pb;
{
	...
	if ((string = newsizedstringobject((char *)&sc_pb->buffer[sc_pb->bp], size))
									== NULL)
		return -1;
	sc_pb->bp += size;
	if (xPop((TscOperand) 1, sc_pb) != 0)
	...
	struct sc_ProcessBlock *sc_pb;
{
	...
	if ((string = newsizedstringobject((char *) &sc_pb->buffer[sc_pb->bp],
					   (int)size)) == NULL)
		return -1;
	sc_pb->bp += (int)size;
	if(xPush(string, sc_pb) != 0) {
...
struct sc_ProcessBlock sc_pb;
	...
	while (sc_pb.pc < datasize) {
...
#ifdef STACK_TRACE
		{
			...
			printf("Stack trace :\n");
			for(i = 0; i < sc_pb.sp; i++) {
				if (printobject(sc_pb.stack[i], stdout, 0) !=0)
					err_clear();
				printf("\n");
			}
		}
